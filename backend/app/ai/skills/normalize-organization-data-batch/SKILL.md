...
3. 对空白做规范化：仅把真正空值、纯空白或明确空占位处理为 `null`；不得猜测姓名、类别、
   编号、班级、电话或邮箱。
4. 第三方部门和老师必须具有类别、姓名、编号、电话、邮箱；第三方学生还必须具有班级。
   任一适用字段缺失，就设置 `invalid=true`，在 `exclusion_codes` 列出原因，并使该行后续对账
   中不可见。第三方异常行仍是不可变证据，最终必须接受 AI 异常分析和报告，但不得生成
   第三方治理操作。
...
- 第三方权威完整性是严格规则：缺一个适用字段就标记并从身份索引排除，不能用希沃补齐权威。
