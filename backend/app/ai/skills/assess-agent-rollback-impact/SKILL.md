...
- create/delete 的补偿会新增或删除整条记录，服务端比较事实因此必须覆盖该连接器会写入或删除
  的完整物理记录字段集合，包括 CSV 自定义列。同步后写入自定义字段必须归类为 `conflict`；
  缺少完整行证据或字段集合时也必须失败关闭，不能把未记录字段当作空值或相等。
...
4. 按原执行事实和真实业务依赖解释补偿：create 的补偿是删除原创建记录；update 只恢复原操作
   影响字段的 `before`；delete 仅在完整物理行 `before` 和连接器支持创建时恢复记录。列表顺序
   或目标版本派生顺序不是业务依赖，不能让一条记录的冲突阻断无依赖记录。
...
- 原 create：当前记录不存在为 `already_restored`；当前记录的完整物理字段仍与原创建后的
  完整记录一致为 `safe_to_restore`；任一规范字段、自定义列或其他会随整行删除的值已变化
  即为 `conflict`，不得删除后续数据。版本 ID 变化但完整记录未变化仍不能单独构成冲突。
- 原 delete：当前记录仍不存在为 `safe_to_restore`；记录已存在且与完整 `before` 一致为
  `already_restored`；记录存在但内容不同为 `conflict`。缺少完整物理 `before` 时不得恢复。
...
- 人工批准绑定操作集合和 `comparison_hash`。批准后、真正写入前若重新读取的相关 `current`
  产生不同哈希，旧批准对该操作失效；执行器必须以 `conflict_skipped` 安全跳过且不写入，
  并在任务结果和报告中明确提示。若仍需回滚，必须基于最新数据重新发起评估和审批；不得虚构
  当前状态图不存在的同任务二次确认入口。
- 规划评估阶段已存在的冲突进入受限人工澄清；执行期新漂移按上一条安全跳过并要求新任务，
  两者都不得用“第三方仍是这样”强行覆盖希沃。
...
- 禁止在 create/delete 的整行补偿中忽略自定义列、未知物理字段或不完整行事实；禁止把列表
  顺序、文件版本链顺序当成业务依赖。
