...
完整链路跨越文件接入、标准化、实体匹配、差异检测、AI 分析、前端审批、执行验证、审计、报告和回滚，因此需要清晰的领域边界。后续 `ai-new-ui` 变更在分析与执行之间增加不可变治理方案，并为外部模型增加任务级令牌化和企业网关配置；私有模型部署不属于本变更。
...
- 对每条差异提供执行前强制成因分析，并把用户选定的 AI 方案或人工修改保存为待执行治理方案。
...
- 模型数据保护由 `ai-new-ui` 的令牌化边界承担；本变更不实现私有模型部署或企业单点登录。
...
`ai-new-ui` 先把用户审核过的 AI 选项或人工修改保存为 `pending_execution` 治理方案。执行模块只消费这些不可变方案，不再从分析文字或浏览器请求临时推导修改内容。计划构建器重新读取差异、分析、目标快照和字段策略，把方案确定性编译为 `create`、`update`、`move`、`disable` 或 `skip` 操作；尚处于 `manual_review` 的差异不可进入批次。

审批分为两道门禁：第一道是单条差异中选择 AI 方案或确认人工修改，第二道是批次级 before/after 预览和确认。第一版允许同一后端操作员完成两次确认，高风险操作必须额外确认；批次模型预留独立审核人字段供未来四眼审批使用。差异检测、风险门禁、依赖排序、预检、执行、幂等、验证、审计和补偿全部由确定性服务负责。

### 6. Constrain Agent and MCP responsibilities

一个组织数据治理 Agent 使用版本化 Skill 完成成因分析、模糊匹配、方案建议、批次解释、历史恢复影响解释和报告生成。报告与恢复解释复用前序 AI 分析的企业网关、模型配置、重试和任务令牌化边界，但各自使用独立 Skill 与结构化 Schema。MCP Server 只暴露读取差异、搜索候选、读取映射规则和读取执行上下文等受控工具。

批次解释可以复用 AI 分析的企业网关和模型，但必须使用独立的只读 Skill、结构化 Schema 和同一令牌化边界。解释是可选信息，模型不可用不得阻塞已审核方案的预览、确认或执行。Agent 输出通过 Pydantic 和业务规则校验；CSV 写入、未来 API 写入和回滚执行不作为 Agent 可自由调用的 MCP 工具，它们必须经过后端审批凭证和 Execution Service。

### 7. Use append-only execution and compensation

治理计划绑定精确的方案 ID/版本、差异版本和目标快照。执行批次保存计划版本、方案创建人、批次确认人、可选独立审核人、预检查结果和逐项状态。每个操作保存 before/after、目标版本、错误与验证结果。批次可部分成功，重试只处理符合条件的失败项。

外部目标不能参与数据库事务，因此历史恢复采用 Saga 式补偿。用户从不可变执行时间线选择目标版本；服务解析当前有效状态与目标历史状态之间的版本路径，从已验证操作事实确定性生成反向或重放操作。AI 可读取中间执行事实和已有报告生成候选计划与解释，但后端必须验证其与权威操作集一致。用户确认后，系统只创建一个新的高风险补偿批次，并复用普通执行、幂等、重试和验证服务。

恢复永远向前追加版本。例如 V3 恢复到 V1 会生成内容等同 V1 的 V4，之后选择 V2 会再生成内容等同 V2 的 V5。系统不移动版本指针、不删除 V2/V3，也不直接撤销补偿批次。原执行记录、目标版本和报告不可修改。
...
- `governance_proposals`：AI 选择或人工修改形成的不可变待执行方案及其版本链。
- `governance_plans` 与 `execution_batches`：批准计划和批次。
- `execution_operations`：逐项 before/after、结果和验证。
- `governance_reports`：版本化报告。
- `restore_requests` 与 `restore_execution_links`：当前版本、目标历史版本、覆盖执行范围、补偿计划/批次和输出版本关系。
...
- [执行解释再次把敏感数据发送给模型] → 复用 `ai-new-ui` 的企业网关、任务级令牌化和只读 MCP 边界，且解释失败不影响确定性执行。

## Migration Plan

1. 创建前后端骨架、数据库迁移和本地 PostgreSQL 开发环境。
2. 上线 CSV 接入、标准模型、快照和基础精确匹配闭环。
3. 增加 Embedding、Agent、Skill、MCP、强制分析以及 AI/人工待执行方案。
4. 增加治理方案批次审核、版本化 CSV 执行和回读验证，完成业务闭环。
5. 增加执行历史、报告和回滚补偿。
