## 1. Establish the task-intent boundary

- [x] 1.1 Add focused tests for non-file task-intent validation, legacy session-payload validation, invalid stored payload fallback, and explicit stale-data cleanup
...
- [x] 1.3 Implement the versioned `sessionStorage` compatibility adapter with runtime validation and no CSV or file-summary persistence
...
- [x] 2.1 Replace current creation-page tests with focused conversation tests for internal context updates, assistant failure recovery, and absence of task settings and CSV controls
- [x] 2.2 Extract the Agent conversation into a dedicated page that never exposes task settings, uploads files, or creates a reconciliation task
- [x] 2.3 Keep recognized intent private to the active conversation without persistence, manual-sync navigation, or a handoff action
- [x] 2.4 Verify a fresh “新建对话” session clears an older stored intent without mutating task history
...
- [x] 3.3 Add tests for legacy-intent cleanup, direct-entry defaults, editable task fields, valid paired CSV summaries, invalid-file preservation, and form readiness
...
- [x] 3.5 Reuse the existing task creation service with one idempotency key, pending-submit locking, failure-state preservation, history refresh, stale-intent cleanup, and navigation to task detail
...
- [x] 5.3 Build the conversation page as an unframed operational layout with restrained headings, readable message rhythm, and a focused composer
...
- [x] 6.1 Update Playwright coverage for desktop and mobile sidebar navigation, Agent-only conversation, independent manual-sync reveal, paired CSV creation, and navigation to the unchanged task stages
