...
本设计为第三方权威数据增加“公共网页链接”接入方式。第一版只支持无需登录的 HTTPS
直链 CSV；希沃目标侧继续使用现有上传或受控本地资源。

## 已确认的产品与架构决策

- 用户可以提交一个公开 HTTPS CSV 下载地址作为第三方权威数据来源。
- 核心能力同时服务于对话入口和手动同步入口，两者复用同一套后端合同，不建立两套流程。
...
- 让用户通过公共 CSV 链接接入第三方权威数据，无需先下载到本地。
...
```http
POST /api/agent/remote-sources
Content-Type: application/json
```

```json
{
  "url": "https://data.example.edu/export/organization.csv"
}
```
...
```json
{
  ...
  "display_origin": "data.example.edu",
  "state": "registered"
}
```

登记阶段验证 HTTPS、URL 语法、禁止的凭据形式和租户身份。实际下载属于同步 Graph，
不在登记请求中执行，以免创建任务前产生不可恢复的长操作。
...
对话入口和手动入口最终都生成相同意图。前端可以分阶段上线入口，但不得建立独立的后端执行
语义。
...
对于已上传 CSV，资源已经固化，该节点直接确认资源可用。对于 `remote_csv`，生成
...
1. 用户可用公开 HTTPS CSV 链接创建第三方权威数据任务。
