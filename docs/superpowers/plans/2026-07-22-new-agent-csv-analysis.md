...
- [ ] **Step 1: Write failing tests** for department/student/teacher projection, authoritative completeness, student class requirement, target rows with only email, no-key target-extra marking, UTF-8/GB18030, stable physical row order, malformed/unmappable schemas, and replay idempotency.
...
- [ ] **Step 1: Write failing tests** for normalized exact postings, duplicate authority hits, contradictory keys, student→teacher→department search evidence, missing-number recovery by phone/email, earliest stable target claim, later duplicate target, no-hit target-extra, unclaimed authority target-missing, invalid authority exclusion, and correct-row silence.
...
- [ ] **Step 1: Write failing tests** for `50/51` partitioning, exact response membership, duplicate/omitted/forged IDs, Chinese category, one-to-three solutions, one recommendation, allowed operations/evidence/values, authority-write rejection, and no partial commit.
...
- [ ] **Step 6: Implement durable analysis service** with single-attempt provider calls inside one-initial-plus-three retry orchestration, sanitized attempt provenance, atomic finding/solution finalization, and blocked-model failure on exhaustion without releasing the school lock.
...
- [ ] **Step 6: Update OpenSpec checkboxes only for fully delivered tasks**: 1.4, 2.3–2.5, 3.1–3.4, 4.4–4.6, and 6.1–6.9 when their complete acceptance tests pass; leave reports, approvals, execution, public API, frontend, API/database connectors and broad all-phase worker tasks pending.
